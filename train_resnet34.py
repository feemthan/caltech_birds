...
def main(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        if args.freeze and epoch in unfreeze_schedule:
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), 'best_resnet34_caltech200.pth')
            print(f"Best model saved with accuracy: {best_acc:.2f}%")

    torch.save(model.state_dict(), 'final_resnet34_caltech200.pth')
    print(f"Training completed. Best accuracy: {best_acc:.2f}%")
    pd.DataFrame(history).to_csv('history.csv', index=False)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train ResNet34 on Caltech200 dataset')
