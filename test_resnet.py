...
def show_misclassified_examples(model, test_loader, device, class_names, num_examples=5):
    ...
    for _, true_label, pred_label, path in misclassified[:num_examples]:
        print(f"Image: {path}")
...
def main(config):
    ...
    tn = np.sum(cm) * (cm.shape[0] - 1) - fn  # TN = all correct predictions for other classes

    plot_2x2_confusion_matrix(tp, fn, fp, tn)
    print("2x2 Confusion matrix saved as '2x2_confusion_matrix.png'")
