...
class Caltech200Dataset(Dataset):
    def __init__(self, root_dir, txt_file, transform=None, num_augmentations=5):
...
def main(args):
    ...
    train_dataset = Caltech200Dataset(root_dir=args.root_dir, 
                                      ...
                                      transform=transform, num_augmentations=4)
    ...
    unfreeze_schedule = {
        int(args.num_epochs/4): 'layer4',
        int(args.num_epochs*2/4): 'layer3',
        int(args.num_epochs*3/4): 'layer2'
    }
    ...
    for epoch in range(args.num_epochs):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), 'best_resnet18_caltech200.pth')
            print(f"Best model saved with accuracy: {best_acc:.2f}%")

    torch.save(model.state_dict(), 'final_resnet18_caltech200.pth')
    print(f"Training completed. Best accuracy: {best_acc:.2f}%")
    pd.DataFrame(history).to_csv('history.csv', index=False)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train ResNet18 on Caltech200 dataset')
